...
function assertScore0to100(value: unknown, label: string): asserts value is { score: number } {
  const score = (value as { score?: unknown } | undefined)?.score
...
function levelFor(score: number): ScoreLevel['level'] {
    ...
    OCEAN_DOMAINS.forEach(domain => {
      ...
      evidence.forEach(quote => {
        if (!findVerbatimEvidence(transcript, quote)) {
          violations.push(
            `ocean.${domain} evidence is not a verbatim transcript substring: "${quote.slice(0, 120)}"`
          )
        }
      })
    })
